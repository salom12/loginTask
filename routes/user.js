...
router.post('/checKlogin', checkLogin, (req, res) => {
    // Didn't use this, I created a (checkLogin) middleware instead
    res.status(200).send('Yay! you passed the checkLogin middleware!');
});
...
router.post('/login', (req, res) => {
  ...
  const validating = credentialsValidating(req.body);
  if(validating.error){
    res.status(400).send(validating.error);
  }else {
    //  check if there is such email get the user info
    User.findOne({email: "hikki.ahmed@gmail.com"}).then(user => {
...
function credentialsValidating(user) {
  const userSchema = {
